...
class Generator(nn.Module):
    def __init__(self, config, use_cuda, device_ids):
        ...
        self.coarse_generator = CoarseGenerator(config['input_dim'], config['cnum'], use_cuda, device_ids)
        self.fine_generator = FineGenerator(config['input_dim'], config['cnum'], use_cuda, device_ids)

    def forward(self, x, mask):
        coarse_img = self.coarse_generator(x, mask)
        fine_img, offset_flow = self.fine_generator(x, coarse_img, mask)
        return coarse_img, fine_img, offset_flow

class Generator_primitive(nn.Module):
    def __init__(self, config, use_cuda, device_ids):
        super(Generator_primitive, self).__init__()
        self.coarse_generator = CoarseGenerator(config['input_dim'], config['cnum'], use_cuda, device_ids)
        self.fine_generator = CoarseGenerator(config['input_dim'], config['cnum'], use_cuda, device_ids)

    def forward(self, x, mask, mask1):
        # Mask 1 is essentially with no holes
        coarse_img = self.coarse_generator(x, mask)
        fine_img = self.fine_generator(x, mask1)
